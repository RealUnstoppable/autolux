...
## 2024-05-24 - Missing Aria-Controls on Toggles
**Learning:** Custom interactive widgets in this app, like the mobile menu toggles and FAQ accordions, are missing the `aria-controls` attribute, which breaks the relationship between the toggle and the content for screen readers.
**Action:** Always ensure toggle buttons use `aria-controls="[id of target container]"` to explicitly link the control to the element it expands or collapses.
