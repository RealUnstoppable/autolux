...
## 2025-05-24 - [sessionStorage Privacy Mode Restrictions]
**Learning:** `sessionStorage` and `localStorage` throw exceptions (like `DOMException: Failed to read the 'sessionStorage' property from 'Window': Access is denied for this document`) in strict privacy or incognito modes in some browsers. Attempting to access or write to them without `try/catch` blocks will cause the entire synchronous or async flow to halt, breaking rendering for subsequent components.
**Action:** When implementing caching mechanisms using `sessionStorage` or `localStorage` to reduce database reads, always wrap the `getItem`, `setItem`, and `JSON.parse` operations in `try/catch` blocks. This ensures the app gracefully falls back to performing standard database network requests rather than failing silently or blocking UI updates.
