...
## 2025-05-25 - [Preload Critical LCP Background Images & Explicit Sizing]
**Learning:** Background images loaded via CSS (like the hero cover) are discovered late by the browser, causing a delayed Largest Contentful Paint (LCP). Additionally, lazy-loaded images without explicit dimensions can cause Cumulative Layout Shift (CLS) as they render.
**Action:** Always use `<link rel="preload" as="image" fetchpriority="high">` in the document `<head>` for critical background images, and explicitly set `width`/`height` attributes on lazy-loaded images to reserve DOM space and eliminate layout shifts.
