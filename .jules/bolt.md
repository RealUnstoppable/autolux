...
## 2025-05-24 - [Firebase Timestamp JSON Serialization Loss]
**Learning:** When caching Firestore documents in `sessionStorage` or `localStorage`, Firebase `Timestamp` objects lose prototype methods like `.toDate()` during JSON serialization. If code attempts to call `.toDate()` on the parsed object, it will throw an error.
**Action:** Ensure timestamps are converted to primitive values (e.g., ISO strings) before caching, or implement fallback logic for parsed objects when retrieving them from storage.
