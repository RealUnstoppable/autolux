...
## 2026-05-20 - [Handle Storage Quota Exceptions gracefully]
**Learning:** Calling `sessionStorage.setItem()` or `localStorage.setItem()` directly within the main network-fetching `try/catch` block can cause the entire feature to break if the user's browser has strict privacy settings (which throw `SecurityError`) or if the storage quota is exceeded (which throws `QuotaExceededError`). If this happens, the `catch` block catches the storage error instead of the network error, breaking the application's fallback behavior.
**Action:** Always wrap `sessionStorage.setItem()` calls in their own independent `try/catch` block, separate from the primary data-fetching logic, so that storage failures fail silently and gracefully fall back to the standard rendering logic.
