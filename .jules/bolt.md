...
## 2025-05-24 - [Avoid Layout Thrashing with DocumentFragment]
**Learning:** When rendering large lists or tables dynamically by appending elements to the DOM inside a loop, the browser has to recalculate the layout and repaint the screen on every iteration. This causes layout thrashing and severely degrades UI performance, especially in heavy pages like the admin dashboard.
**Action:** Always construct the entire list or table in memory using `document.createDocumentFragment()` during the loop, and then append the complete fragment to the DOM in a single operation after the loop has finished.
