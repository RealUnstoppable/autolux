...
## 2026-12-15 - [Privilege Escalation via Mass Assignment in Reviews]
**Vulnerability:** The `reviews` collection in `firestore.rules` allowed any authenticated user to create a review without schema validation or constraints on the `status` field.
**Learning:** Failing to restrict administrative fields (like `status`) during document creation allows users to bypass approval queues (e.g., setting `status: 'approved'`) and publish untrusted content directly.
**Prevention:** Always enforce strict schema validation for user-created documents in `firestore.rules` using `request.resource.data.keys().hasOnly([...])` and explicitly restrict administrative fields (e.g., `(!('status' in request.resource.data) || request.resource.data.status == 'pending')`).
