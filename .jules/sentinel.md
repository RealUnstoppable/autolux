...
## 2024-06-01 - Privilege Escalation in Firestore Users Collection
**Vulnerability:** In `firestore.rules`, the `allow update` and `allow create` rules for the `/users/{userId}` collection allowed users to modify *any* field on their own document, including sensitive role fields like `isAdmin`. This allowed a malicious user to trivially grant themselves administrative privileges by adding `isAdmin: true` to a profile update payload.
**Learning:** Firestore rules that only check ownership (`request.auth.uid == userId`) are insufficient for documents that contain authorization claims (like roles or permissions). Relying on the client to honestly submit fields leads to privilege escalation.
**Prevention:** For any collection containing role-based data (especially users/profiles), explicitly deny modification of sensitive fields during `create` and `update` using rules like `!request.resource.data.diff(resource.data).affectedKeys().hasAny(['isAdmin'])`. Role changes should only be permitted by existing admins or trusted server-side Cloud Functions.
