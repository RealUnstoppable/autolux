...
## 2026-05-28 - [Privilege Escalation via Insecure Firestore Rules]
**Vulnerability:** A critical Privilege Escalation vulnerability was found in `firestore.rules`. The `match /users/{userId}` collection previously had an `allow update: if isOwner(userId) || isAdmin();` rule. Because `isAdmin` authorization checking relied entirely on `data.isAdmin == true` within the same document, any authenticated user could submit an `update` to their own document setting `{ isAdmin: true }` to gain full admin privileges across the application.
**Learning:** Overly permissive `update` rules on user documents that store authorization roles (like `isAdmin`) allow users to promote themselves.
**Prevention:** When storing roles in the same document as the user profile, explicitly exclude those role fields from user-initiated updates. Use `!request.resource.data.diff(resource.data).affectedKeys().hasAny(['isAdmin'])` for `allow update` and prevent it during `allow create`.
