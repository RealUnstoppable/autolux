...
## 2026-05-25 - [Privilege Escalation via Firestore Rules]
**Vulnerability:** A critical Privilege Escalation vulnerability existed in `firestore.rules`. The `match /users/{userId}` rules allowed `update` and `create` actions based solely on `isOwner(userId)`. This allowed any authenticated user to create or update their own user document and include `"isAdmin": true`, granting themselves application-wide admin access.
**Learning:** Checking ownership is not enough when certain document fields dictate access control. If users have unconstrained write access to their own documents, they can manipulate sensitive role-based fields.
**Prevention:** Always restrict write access to sensitive fields. For `create`, use `request.resource.data.get('isAdmin', false) == false`. For `update`, use `!request.resource.data.diff(resource.data).affectedKeys().hasAny(['isAdmin'])` to ensure users cannot modify their privileges unless they are already an admin.
