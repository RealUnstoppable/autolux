...
## 2024-05-24 - Firestore Wildcard Privilege Escalation
**Vulnerability:** A catch-all wildcard rule (`match /{document=**}`) contained `allow` statements, granting global privileges and bypassing specific collection restrictions.
**Learning:** In Firestore rules, any rule that evaluates to true allows access, evaluating using a logical OR. A wildcard with an `allow` statement thus overrides specific collection denials.
