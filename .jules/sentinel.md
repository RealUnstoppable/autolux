...
## 2024-05-24 - Privilege Escalation via Wildcard Rules
**Vulnerability:** A catch-all wildcard rule (`match /{document=**}`) contained permissive `allow` statements, granting global privileges and bypassing specific collection restrictions.
**Learning:** In Firestore rules, a catch-all wildcard rule that contains an `allow` statement evaluates using a logical OR, which can unintentionally override stricter rules on specific collections.
**Prevention:** Always secure catch-all rules by explicitly denying access (`allow read, write: if false;`).
