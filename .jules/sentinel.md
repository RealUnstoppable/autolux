...
## 2024-05-24 - Privilege Escalation in Firestore Wildcard Rule
**Vulnerability:** A catch-all wildcard rule (`match /{document=**}`) contained `allow create` and `allow update, delete` statements.
**Learning:** In Firestore rules, a catch-all wildcard rule evaluates using a logical OR, granting global privileges and bypassing specific collection restrictions.
**Prevention:** Always secure catch-all rules by explicitly denying access (`allow read, write: if false;`). Do not add allow rules to a catch-all unless intentionally giving global access.
