...
## 2026-07-22 - Global Privilege Escalation via Wildcard Rule
**Vulnerability:** A catch-all wildcard rule (`match /{document=**}`) contained `allow create` and `allow update, delete` statements granting privileges.
**Learning:** In Firestore rules, catch-all wildcard rules evaluate using a logical OR, granting global privileges and bypassing specific collection restrictions.
