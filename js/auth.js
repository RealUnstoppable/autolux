...
try {
    ...
    // Use window.location.hostname to dynamically set authDomain and projectId to prevent cross-contamination
    const hostname = window.location.hostname;

    const firebaseConfig = {
        apiKey: window.ENV?.FIREBASE_API_KEY,
        authDomain: window.ENV?.FIREBASE_AUTH_DOMAIN || (hostname === "localhost" || hostname === "127.0.0.1" ? "autolux.realunstoppable.store" : hostname),
    ...
    if (!firebaseConfig.apiKey) {
        console.warn("Firebase API Key is missing. Check your window.ENV configuration.");
    }
