...
try {
    ...
    const isAutolux = hostname.includes('autolux') || hostname === 'localhost' || hostname === '127.0.0.1';

    const firebaseConfig = {
        apiKey: window.ENV?.FIREBASE_API_KEY,
        authDomain: isAutolux ? "autolux.realunstoppable.store" : window.ENV?.FIREBASE_AUTH_DOMAIN,
        projectId: isAutolux ? "autolux-detailing" : window.ENV?.FIREBASE_PROJECT_ID,
        storageBucket: isAutolux ? "autolux-detailing.appspot.com" : window.ENV?.FIREBASE_STORAGE_BUCKET,
