...
// Looking for where users are fetched
content = content.replace(/const usersSnap = await getDocs[\s\S]*?renderUsersTable\(users\);/, match => match + quotesFetchScript);

// Also verify if renderQuotesTable is already there
if(content.includes('function renderQuotesTable(quotes) {') && content.includes('const details = q.details || q.vehicleDetails || \'Not specified\';') == false) {
    content = content.replace(/function renderQuotesTable\(quotes\) \{[\s\S]*?tbody\.appendChild\(row\);\n            \}\);\n        \}/, quoteTableScript);
}
